...
const typeNum = (arr) => {
  // Solution code here...
  const onlyNumbers = arr.filter((number) => typeof number === 'number');
  return onlyNumbers;
};
...
const containsAnd = (arr) => {
  // Solution code here...
  const onlyAnd = arr.filter((string) => string.includes('and'));
  return onlyAnd;
};
...
const oddValues = (arr) => {
  // Solution code here...
  const onlyOdd = arr.filter((integer) => integer % 2 ===1);
  return onlyOdd;
};
