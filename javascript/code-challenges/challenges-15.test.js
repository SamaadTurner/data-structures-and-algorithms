...
const toTitleCase = (arr) => {
  ...
  return arr.map((str) => {
    if (str.length === 0) {
      // Handle empty strings, return them as is
      return str;
    }
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let lukeMass = 77;
  const newArr = arr.filter((character) => {
    if (parseInt(character.mass) > lukeMass) {
      return true;
    }
    return false;
  });
  return newArr.map(character => character.name).join(' - ');
};
...
const isSecure = (url) => {
  ...
  // Check if the URL matches the secure URL pattern
